...
public class LimitViolationManager {
    ...
    public void detectViolations(LfNetwork network, Predicate<LfBranch> isBranchDisabled, List<BranchLimitsToCheck> branchLimitsToCheck) {
        detectViolations(network, isBranchDisabled, branchLimitsToCheck, true);
    }

    /**
     * Detect violations on branches and on buses, checking only the branches that carry at least one limit, and
     * optionally skipping the network-wide bus voltage scan. Combines the two reductions: a DC security analysis both
     * reuses the precomputed limit groups and leaves the bus voltages undefined.
     * @param network network on which the violation limits are checked
     * @param isBranchDisabled predicate to evaluate if a branch of the network is disabled or not
     * @param branchLimitsToCheck the precomputed limit groups of the branches carrying at least one limit
     * @param detectBusVoltageViolations whether bus voltage violations should be looked for; a DC security analysis
     ...
     */
    public void detectViolations(LfNetwork network, Predicate<LfBranch> isBranchDisabled,
                                 List<BranchLimitsToCheck> branchLimitsToCheck, boolean detectBusVoltageViolations) {
        Objects.requireNonNull(network);

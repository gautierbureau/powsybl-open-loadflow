...
public abstract class AbstractSecurityAnalysis<V extends Enum<V> & Quantity, E extends Enum<E> & Quantity,
    ...
    SecurityAnalysisReport runSync(SecurityAnalysisParameters securityAnalysisParameters, ContingenciesProvider contingenciesProvider,
        ...
        if (securityAnalysisParametersExt.getThreadCount() == 1) {
            ...
            // the round-robin partitions as its fallback (multi-component / DC), so both need the ordered result/report merge
            boolean roundRobinPartitioning = partitioningMode != OpenSecurityAnalysisParameters.ContingencyPartitioningMode.SLICE;
            // shared-queue dynamic work stealing: AC single-component only (checked after the build), other cases fall
            // back to the round-robin partitions
            ...
            // the security analysis run phase never reads the iidm network from the worker threads:
            // the limits caches and the bus derived data are materialized before the copies are
            // taken, the branch nominal voltages and voltage level ids are cached at build time, and
            // the action conversion uses precomputed power shifts. This lets the copy mode run on
            // iidm implementations that do not support multi thread access at all (e.g.
            // powsybl-network-store), whatever the analysis uses.
     ...
     * full contingency list (all workers share it) used to index the operator strategies; the per-worker

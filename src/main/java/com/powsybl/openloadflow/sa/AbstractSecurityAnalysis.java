...
public abstract class AbstractSecurityAnalysis<V extends Enum<V> & Quantity, E extends Enum<E> & Quantity,
    ...
    SecurityAnalysisReport runSync(SecurityAnalysisParameters securityAnalysisParameters, ContingenciesProvider contingenciesProvider,
        ...
        if (securityAnalysisParametersExt.getThreadCount() == 1) {
            ...
            // the action conversion uses precomputed power shifts. No need for the variant multi
            // thread access mode, which lets the multi-threaded analysis run on IIDM implementations
            // that do not support it (e.g. powsybl-network-store)
            ContingencyMultiThreadHelper.buildOnceCopyAndRunAnalysis(network, workingVariantId, contingenciesPartitions, creationParameters, topoConfig,
                    parameterProvider, presolver, networksPreparer, contingencyRunner, saReportNode, reportMerger, roundRobinPartitioning, executor);

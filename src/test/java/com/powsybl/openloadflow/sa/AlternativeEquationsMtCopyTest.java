 ...
 * that gives every contingency partition its own deep {@code LfNetworkCopier} copy of the network (the only network
 * per thread mode after the copy-only rework), for the three contingency distribution modes:
 * {@link OpenSecurityAnalysisParameters.ContingencyPartitioningMode#SLICE} (contiguous),
 * {@link OpenSecurityAnalysisParameters.ContingencyPartitioningMode#ROUND_ROBIN} (interleaved) and
 * {@link OpenSecurityAnalysisParameters.ContingencyPartitioningMode#SHARED_QUEUE} (dynamic work stealing).
 *
 * <p>This exercises the two SA hooks that both features add to {@code AbstractSecurityAnalysis}, on the copied
 * (and, when supported, presolved) networks of each partition/worker: {@code adaptParameters} (which computes the
 * alternative islandable bus ids from the copy and its contingencies) and {@code initStateFromPresolvedNetwork}.
 * With alternative equations the matrix structure is preserved even when a contingency islands an eligible bus, so
 * a worker keeps reusing its single Jacobian analysis across every contingency it processes.
 *
 * <p>The multi-threaded copy-mode results with alternative equations must agree with the single-threaded ones to
 * within solver tolerance whatever the distribution (the copy feature alone is bit-identical, but with alternative
...
class AlternativeEquationsMtCopyTest extends AbstractOpenSecurityAnalysisTest {
    ...
    private static List<Contingency> contingencies() {
        // several contingencies so ROUND_ROBIN and SHARED_QUEUE actually interleave them across workers:
    ...
    void alternativeEquationsCopyModeIdenticalToSingleThread(int threadCount, OpenSecurityAnalysisParameters.ContingencyPartitioningMode partitioningMode) {
        // multi-threaded copy-mode SA with alternative equations must match the single-threaded one, for every
        // contingency distribution: the deep copy of each worker carries the same network, and the per-worker
        // adaptParameters computes the same alternative islandable bus ids
        SecurityAnalysisResult singleThread = run(createEurostagNetworkWithIslandableGenerator(), true, 1, partitioningMode);
        SecurityAnalysisResult multiThread = run(createEurostagNetworkWithIslandableGenerator(), true, threadCount, partitioningMode);
    ...
    void alternativeEquationsCopyModeIdenticalToLegacy(int threadCount, OpenSecurityAnalysisParameters.ContingencyPartitioningMode partitioningMode) {
        ...
        // given distribution, must give the same result as the legacy modeling run single-threaded
        SecurityAnalysisResult legacy = run(createEurostagNetworkWithIslandableGenerator(), false, 1,
                OpenSecurityAnalysisParameters.ContingencyPartitioningMode.SLICE);
        SecurityAnalysisResult alternativeMultiThread = run(createEurostagNetworkWithIslandableGenerator(), true, threadCount, partitioningMode);
